...
#Create Income_Group as a categorical variable
gas_data <- gas_data %>%
...
#Perform t-test to assess difference in means
...
#Summary statistics by stoplight presence
summary_stats <- gas_data %>%
  group_by(Stoplight) %>%
  summarize(
...
### **Evidence:**
```{r echo=FALSE, message=FALSE, fig.align='center', fig.width=4, fig.height=2}
# Convert 'Highway' column to a factor for analysis
...
# Summary statistics for Shell vs. Other Brands
summary_stats <- df %>%
...
# Visualization: Boxplot of Gas Prices by Brand
...
# Filter dataset for the two shows
...
# Compute means, standard deviations, and sample sizes
...
# Filter dataset for "Dancing with the Stars"
df_C <- df %>% filter(Show == "Dancing with the Stars")

# Compute proportion of viewers who found it confusing (Q2_Confusing >= 4)
confused_count <- sum(df_C$Q2_Confusing >= 4, na.rm = TRUE)
total_count <- sum(!is.na(df_C$Q2_Confusing))
p_hat <- confused_count / total_count

# Compute standard error and confidence interval for the proportion
...
# Print table using kable for formatting
...
# Compute observed difference in mean revenue ratios
